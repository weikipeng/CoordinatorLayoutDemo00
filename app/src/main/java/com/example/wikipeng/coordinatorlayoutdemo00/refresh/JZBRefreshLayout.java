 ...
 */
public class JZBRefreshLayout extends LinearLayout {
    private static final String TAG = BGARefreshLayout.class.getSimpleName();

    private JZBRefreshViewHolder mRefreshViewHolder;
    /**
     ...
     * 下拉刷新和上拉加载更多代理
     */
    private JZBRefreshLayoutDelegate mDelegate;
    /**
     * 手指按下时，y轴方向的偏移量
     */
    ...
    private JZBRefreshScaleDelegate mRefreshScaleDelegate;

    private int mTouchSlop;

    public JZBRefreshLayout(Context context) {
        this(context, null);
    }

    public JZBRefreshLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
...
//            mStickyNavLayout.setRefreshLayout(this);
    ...
    public void setRefreshViewHolder(JZBRefreshViewHolder refreshViewHolder) {
...
//                            FOpenLog.e("jzbFocus debug JZBRefreshLayout ===> onInterceptTouchEvent "
//                                    + JzbWebView.getEventName(event) + " 拦截 <<<<<<<<<<<<<<<<<< ");
                            return true;
                        }
                    }
                }
...
//            FOpenLog.e("jzbFocus debug JZBRefreshLayout ===> onInterceptTouchEvent "
//                    + JzbWebView.getEventName(ev) + "<<<<<<<<<<<<<<全分发 true========================== ");
            return true;
        }

//        FOpenLog.e("jzbFocus debug JZBRefreshLayout ===> onInterceptTouchEvent "
//                + JzbWebView.getEventName(ev) + "============super.dispatchTouchEvent========================== ");
    ...
    private boolean isContentViewToTop() {
        ...
        if (BGARefreshScrollingUtil.isRecyclerViewToTop(mRecyclerView)) {
            if (mRecyclerView instanceof JzbRecyclerView) {
                boolean isIntercept = ((JzbRecyclerView) mRecyclerView).isInterceptMoveDown(null);
//                FOpenLog.e("jzbFocus debug JZBRefreshLayout ===> 刷新 isContentViewToTop  " + !isIntercept);
                return !isIntercept;
            }
            return true;
        }
            ...
            switch (event.getAction()) {
                case MotionEvent.ACTION_DOWN:
                    ...
                    if (isWholeHeaderViewCompleteInvisible()) {
                        mRefreshDownY = (int) event.getY();
//                        FOpenLog.e("jzbFocus debug JZBRefreshLayout ===> onTouchEvent  "
//                                + JzbWebView.getEventName(event) + "处理<<<<<<<<<<<<<<<<<<<");
                        return true;
                    }
                    break;
                case MotionEvent.ACTION_MOVE:
...
//        FOpenLog.e("jzbFocus debug JZBRefreshLayout ===> onTouchEvent  "
//                + JzbWebView.getEventName(event) + "============不消耗========================== ");
    ...
    public void beginRefreshing() {
        if (mCurrentRefreshStatus != RefreshStatus.REFRESHING && mDelegate != null) {
            ...
            handleRefreshStatusChanged();
            mDelegate.onJZBRefreshLayoutBeginRefreshing(this);
        }
    }
    ...
    public void beginLoadingMore() {
        if (!mIsLoadingMore && mLoadMoreFooterView != null && mDelegate != null && mDelegate.onJZBRefreshLayoutBeginLoadingMore(this)) {
            mIsLoadingMore = true;
     ...
     * @param delegate
     */
    public void setDelegate(JZBRefreshLayoutDelegate delegate) {
        mDelegate = delegate;
    }
     ...
     */
    public void setRefreshScaleDelegate(JZBRefreshScaleDelegate refreshScaleDelegate) {
    ...
    public interface JZBRefreshLayoutDelegate {
        /**
         * 开始刷新
         */
        void onJZBRefreshLayoutBeginRefreshing(JZBRefreshLayout refreshLayout);
         ...
         */
        boolean onJZBRefreshLayoutBeginLoadingMore(JZBRefreshLayout refreshLayout);
    }

    public interface JZBRefreshScaleDelegate {
        /**
         * 下拉刷新控件可见时，处理上下拉进度
         *
